...
let varietyBonus = 0.3; // your percentage
let tasteBonus = 0.0; // your percentage
...
let avgNutrition = 0;
let ignoreTasteBonus = false; // true to include foods you don't like
let acceptableTotal = 250;
let maxPlates = 5;
let ignoreLowFoods = false;
let minPreference = 0; // make this as Enum
/* NUTRITION LIMITS */
...
let data = {
    ...
    recipeList: [
        ...
        // ["gathering", "agave", 0, 2, 1, 1, 4, 200, 0, [["agave"]]],
        // ["gathering", "beans", 0, 1, 4, 3, 0, 150, 0, [["amanita"]]],
        // ["gathering", "beet", 0, 2, 0, 2, 4, 230, 0, [["beet"]]],
        // ["gathering", "beet greens", 0, 3, 1, 0, 4, 100, 1, [["beet"]]],
        // ["gathering", "bolete", 0, 2, 4, 1, 1, 200, 0, [["bolete"]]],
        // ["gathering", "camas bulb", 0, 1, 2, 5, 0, 150, 0, [["camas bulb"]]],
        // ["gathering", "cookeina", 0, 2, 4, 1, 1, 200, 0, [["bolete"]]],
        // ["gathering", "corn", 0, 4, 1, 0, 3, 230, 0, [["corn"]]],
        // ["gathering", "crimini", 0, 2, 4, 1, 1, 200, 0, [["crimini"]]],
        // ["gathering", "fiddleheads", 0, 2, 1, 0, 5, 150, 0, [["fern"]]],
        // ["gathering", "fireweed shoots", 0, 3, 1, 0, 4, 150, 0, [["fireweed shoots"]]],
        // ["gathering", "giant cactus fruit", 0, 2, 0, 2, 4, 100, 0, [["fireweed shoots"]]],
        // ["gathering", "heart of palm", 0, 4, 2, 0, 2, 100, 0, [["fireweed shoots"]]],
        // ["gathering", "huckleberry", 0, 2, 0, 0, 6, 150, 0, [["huckleberry"]]],
        // ["gathering", "papaya", 0, 2, 1, 0, 5, 200, 0, [["papaya"]]],
        // ["gathering", "pineapple", 0, 6, 0, 0, 2, 200, 0, [["pineapple"]]],
        // ["gathering", "prickly pear fruit", 0, 2, 1, 1, 4, 190, 0, [["prickly pear fruit"]]],
        // ["gathering", "pumpkin", 0, 5, 1, 0, 2, 340, 0, [["pumpkin"]]],
        // ["gathering", "rice", 0, 7, 1, 0, 0, 150, 0, [["rice"]]],
        // ["gathering", "sunflower", 0, 2, 2, 4, 0, 50, 0, [["pumpkin"]]],
        // ["gathering", "taro", 0, 6, 1, 0, 1, 250, 0, [["pumpkin"]]],
        // ["gathering", "tomato", 0, 5, 1, 0, 2, 240, 0, [["tomato"]]],
        // ["gathering", "wheat", 0, 6, 2, 0, 0, 150, 0, [["wheat"]]],
    ],
    _forReference: {
        multiple: 1,
        gathering: 1,
...
function restrictMenu(menu, sources) {
    ...
    if (debug) (console.log(`total meals: ${menu.length}`))

    // ignore taste bonus...
    if (ignoreTasteBonus) menu.forEach(x => x.pref = minPreference);
    //remove unwanted menu
    menu = menu.filter(x => x.pref >= minPreference);

    if (debug) (console.log(`taste bonus: ${menu.length}`))
    ...
    if (debug) (console.log(`low foods: ${menu.length}`))
    ...
    if (debug) (console.log(`skill levels: ${menu.length}`))

    // keep min average nutrition
    menu = menu.filter(x => x.nutritionAverage() > avgNutrition);

    if (debug) (console.log(`nutrition limits: ${menu.length}`))
...
function calc(meal1, count1, meal2, count2, meal3, count3, meal4, count4) {
    ...
    if (meal.calories > (calorieLimit * howManyServings))
        return 0;
...
function count(meal1, meal2, meal3, meal4, max) {
    try {
        ...
        let numPlates = maxPlates * howManyServings;
        let limit1 = Math.min(Math.ceil(calorieLimit * howManyServings / meal1.calories), numPlates);
        let limit2 = Math.min(Math.ceil(calorieLimit * howManyServings / meal2.calories), numPlates);
        let limit3 = Math.min(Math.ceil(calorieLimit * howManyServings / meal3.calories), numPlates);
        let limit4 = Math.min(Math.ceil(calorieLimit * howManyServings / meal4.calories), numPlates);
        console.log(`${calorieLimit * howManyServings}, ${limit1}, ${limit2}, ${limit3}, ${limit4}`)
